...
interface SidebarProps {
  /** Ruta actual (ej: "/expenses"). Se usa para resaltar el item activo. */
  currentPath: string;
  /** Callback opcional al navegar (ej: cerrar menú móvil). */
  onNavigate?: () => void;
  isOpen: boolean;
  onClose: () => void;
}
...
const menuItems: {
  ...
  { path: '/', label: 'Dashboard', icon: LayoutDashboard, color: 'bg-purple-200' },
  { path: '/expenses', label: 'Gastos', icon: Wallet, color: 'bg-red-200' },
  { path: '/income', label: 'Ingresos', icon: TrendingUp, color: 'bg-green-200' },
  { path: '/categories', label: 'Categorías', icon: Tags, color: 'bg-yellow-200' },
  { path: '/reports', label: 'Reportes', icon: BarChart3, color: 'bg-blue-200' },
  { path: '/budgets', label: 'Presupuestos', icon: PiggyBank, color: 'bg-pink-200' },
  { path: '/goals', label: 'Metas', icon: Target, color: 'bg-indigo-200' },
  { path: '/settings', label: 'Configuración', icon: Settings, color: 'bg-gray-200' },
];
...
export default function Sidebar({
  ...
  // Render del item de menú, reutilizable para móvil y desktop
  const renderMenuItem = (item: (typeof menuItems)[number]) => {
    const Icon = item.icon;
    // Resaltamos "/" solo si estamos exactamente en la raíz;
    // los demás, con startsWith para soportar rutas anidadas futuras.
    const isActive =
      item.path === '/'
        ? currentPath === '/'
